...
def post(path, data, role="coordinator"):
    r = httpx.post(f"{API}{path}", json=data, headers=auth_headers(role), timeout=10)
    return r.json()


def get(path, role="coordinator"):
    r = httpx.get(f"{API}{path}", headers=auth_headers(role), timeout=10)
    return r.json()


def patch(path, data, role="coordinator"):
    r = httpx.patch(f"{API}{path}", json=data, headers=auth_headers(role), timeout=10)
    return r.json()
...
def seed():
    ...
    if db.query(Incident).count() > 0:
        db.close()
        print("  Baza zawiera już dane — pomijam seedowanie.")
        return
